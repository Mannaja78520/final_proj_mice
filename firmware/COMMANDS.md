...
**A board answers only the commands its build carries.** Since 2026-08-09 there
is one binary per module type (`pio run -e mice_nong` / `-e mice_lift` /
`-e mice_blank`), so this reference is the union: the core commands are on every
board, and a module's commands are only on a board built for that module.
`HELP` on the board itself always tells the truth for that board, and
`SET TYPE` only accepts a type its binary can boot.
...
**Every module in `ON` mode hosts that AP all the time — including while it is
connected to the show network.** It is not only a fallback:

* the credentials set with `SET WIFI` are always what gets tried, ahead of
  anything compiled into `config.h` (those are a factory fallback only);
* a module parked out of router range can join a *nearby module's* AP and
  reach the network through it, so one board in a good spot covers the ones
  around it;
* and a module you cannot reach at all is still reachable at `192.168.4.1`
  to fix its settings — which is the whole point of never needing a reboot.

Nothing about the radio needs a reboot any more. `WIFI` shows where it is and
`WIFI ON|OFF|AP|RETRY|SCAN|CLEAR` changes it on the spot, because a show cannot be
power-cycled to correct a typo'd network name.

### Groups — which modules are allowed to link

A module's fallback hotspot password is **derived from its group name**, so
modules in one group share it automatically and nothing has to be typed twice:

```
> GROUP mice-show
OK group "mice-show" appass=3f9a1c0b7e2d4a55
```

That is what makes automatic linking safe. With no venue WiFi, a grouped module
scans, finds a neighbour's hotspot and joins it **without ever having met it** —
because the only hotspot it can actually get into is a group-mate's. Two
companies working the same site each let their own modules find each other, and
neither can touch the other's.

An **ungrouped** module never goes hunting. It would be using the shared
compiled-in password, which proves nothing about who owns the board, so it
stays on its own hotspot and waits — exactly as before.

Set it from the hub's **Network** tab (tick the modules that belong together,
give the group a name) or with `GROUP` on any channel.

### Reaching a module through another one

A module parked out of router range joins a nearby module's AP — but it is then
on that module's private subnet, where the PC cannot see it. The module it
joined forwards for it:

```
PC --USB--> lift-test --its own AP--> nong
> PEERS                       # what can this module see?
[{"id":85,"name":"lift-test",...},{"id":67,"name":"nong","ip":"192.168.4.2"}]
> REACH nong PING             # ...and talk to it
PONG 67 nong nong
```

**A weak link is not a failed link.** A module far from the router still
*connects* — badly, and a show stutters. So signal strength is checked too: if
the show network is weak (below about -78 dBm) and a neighbouring module's
hotspot is clearly stronger, the module joins the neighbour and is reached
through it. It returns to the show network once that is properly good again
(-67 dBm), not merely less bad — the two thresholds differ on purpose, so a
module sitting near the boundary cannot flip between networks forever.

`WIFI` shows `via="<module>"` while a module is leaning on a neighbour. The
rule lives in `src/core/WifiLink.h` and is unit-tested on a PC, because a real
-80 dBm signal cannot be produced on a bench.

**One cable is enough for a whole installation with no venue WiFi.** Plug the
PC into any one module and let the others join *that* module's hotspot. The hub
addresses them as `usb:<port>@<name>` — over this cable, then through it to the
module called `<name>` — and wraps every call in `REACH`. The module's website,
Nong Studio, file upload and sequence playback all work on a module the PC has
no route to at all, because only the device string changed.

`REACH` takes an ip, a module name or a bus id, and gives back exactly what
that module answered — one command in, one reply line out, the same shape as
the RS485 bridge (`#3 GOTO 2`). Nothing upstream has to learn a new protocol.

It is for **finding and configuring** a module, not for driving one in a show:
it blocks for up to 800 ms waiting for the far module, so it refuses with
`ERR busy` while this module is moving rather than stall a joint mid-move. In
a show, address the module directly or use RS485.
...
**Reply framing on serial.** The module logs to the same line it answers on, so
every reply is written as one `"\n" + reply + "\n"` — a blank line, the reply,
a newline. Read it as: skip `[tag]` log lines, and take the first line that has
a **blank line immediately before it**. That blank line is what tells a real
...
| `SET TYPE <lift\|nong\|blank>` | `OK ... (reboot to apply)` | which module code runs after reboot. Only types this binary was BUILT with are accepted — on a `mice_nong` board, `SET TYPE lift` answers `ERR unknown type (nong,blank)` |
| `SET WIFI <ssid> [pass]` | `OK joining "..." now (WIFI for progress)` | **quote an ssid containing spaces**: `SET WIFI "MSI 3058" mypass`. Omit pass for an open network |
| `SET WIFI ON\|OFF\|AP` | `OK wifi off — applied now, no reboot` | radio mode: `OFF` = no WiFi at all (USB/RS485 only, fastest boot), `AP` = always host own hotspot, `ON` = join network with AP fallback (default) |
...
| `WIFI ON\|OFF\|AP` | `OK wifi on — applied now, no reboot` | same as `SET WIFI ON\|OFF\|AP`; **takes effect immediately** |
...
same identity, same command language, same website, same RS485 address, same
update over WiFi. What it cannot do is move anything — the camera and the
on-board SD card between them use every usable GPIO, so a camera board can
never also drive a servo. That is why `cam` is its own module type.

Its pins are **not** configurable. Every other pin in this firmware lives in
NVS because a lift or a humanoid is wired by whoever builds it; the camera is
wired by the factory, and "changing" one of those pins could only break it.
They are compile-time constants in `config/esp32_hardware_cam_module.h` and the
Hardware pins page does not offer them.

| Command | Reply | Notes |
|---|---|---|
| `SNAP` | `OK captured 24310 bytes (...)` | take one picture and report its size — proves the sensor works |
| `SNAP <file>` | `OK saved /photos/front.jpg` | save it to the SD card (`/photos`, `.jpg` added if missing) |
| `CAM` | `CAM size=svga quality=12 flash=off vflip=0 hmirror=0 psram=1 shots=3` | what the sensor is set to |
...
big for a one-line reply, which is what RS485 and serial are. Those channels get
`SNAP` instead — it reports the size and where the file went.

`CAM` reports `psram=0` on a board whose PSRAM is missing or broken. That is not
cosmetic: without it the driver keeps one small frame buffer instead of two full
ones, so sizes above SVGA are refused rather than failing at capture time.
...
A board plays a sequence on its **own clock** — that is what lets a show
survive the browser being closed. An app (Nong Studio, the module website, your
own script) can be sending poses down the same cable at the same time. Both
would then drive the same joints, the moves overlap, and the arm fights itself.

So **a command that moves the robot stops the sequence first**:
...
Whoever moved the robot last owns it. The reply says so, on the same line, so
an app cannot silently lose track of what the board is doing.

Which commands count is declared once, in `config/commands.json`
(`"motion": true`), and compiled into the same table `HELP` reads:
...
that is the point of the second column.

The sequence player's own steps do not stop the sequence they came from
(`CommandRouter::handleFromSequence`). And on a two-board humanoid the leader's
forwarded poses arrive at the follower as ordinary commands, so the follower
drops its own sequence and follows the leader — which is what "leader" means.
