...
| `RANGE [<1-8\|name\|ALL> <deg>]` | `OK servo travel L_SH_P = 270 deg` | **per-joint SERVO travel**: 180 for a normal servo, 270 for a wide-angle one (60–360); no args reports all 10 |
| `SERVO [<1-10\|name\|ALL> <type>]` | `OK WAIST = tiankong35 (500-2500us, 250 deg/s, 270 deg travel)` | apply a servo preset: `mg90s`, `pdi1181mg` (270°), `tiankong35` (270°), `generic180`, `generic270` (or set any other servo with `PULSE` + `RANGE`) |

**Joint limits + gear are dynamic and persistent.** Angles are **joint
degrees**; each joint is clamped to its own `[min,max]` (a 2-servo universal
joint binds near the ends — full 0–180 is not reachable, default 30–150).
`LIMIT`/`GEAR`/`PULSE`/`RANGE`/`SERVO` change them live and write
`/data/nong_cal.yaml` on the SD card (loaded at boot, overriding
`module.yaml`), so tuning from Nong Studio sticks.
