    ...
    <div class="setupCard" style="display:none;border-top:1px solid var(--line);margin-top:12px;padding-top:10px">
      ...
      <div class="row">
        ...
        <select id="svType" style="flex:1">
          <option value="mg90s">MG90S (180&deg;, 500-2400us)</option>
          <option value="pdi1181mg">PDI-1181MG (270&deg;, 500-2500us)</option>
          <option value="tiankong35">TianKongRC 35kg (270&deg;, 500-2500us)</option>
      ...
      <div class="row">
        ...
        <input type="number" id="svRange" min="60" max="360" step="10" value="180" style="width:80px"
               title="servo travel in degrees — 180 normal, 270 wide-angle">
        <button onclick="setRange()">Set travel&deg;</button>
        <span class="statline" id="svNow"></span>
      </div>
      <div class="statline">after changing travel the arm stays put at home; check the
        movement away from home and press <b>Set zero</b> again if it needs re-trimming.</div>
    </div>
  </div>
...
function render(){
  ...
  const isLift=st.type==='lift',isNong=st.type==='nong';
  $('liftCard').style.display=isLift?'':'none';
  $('nongCard').style.display=isNong?'':'none';
  $('rgbCard').style.display=isLift?'':'none';
  $('audCard').style.display=isLift?'':'none';
  if(st.wifi){
  ...
  // servo travel readout: only while the user is not typing in the box
  if(isNong&&m.servo_range&&!$('svNow').dataset.held){
    const j=$('svJoint').value, all=m.servo_range;
    const same=all.every(v=>v===all[0]);
    $('svNow').textContent='now: '+(same?(all[0]+'° (all joints)'):all.join(', ')+'°');
...
async function loadPins(){
  ...
  try{
    pinValid=JSON.parse((vt||'').trim());pinCur=JSON.parse((ct||'').trim());
    ...
    $('pinGroups').innerHTML='<div class="statline" style="color:var(--warn)">'+
      ...
      '(pio run -e lift_module -t upload). reply: '+((''+vt+' / '+ct).slice(0,80))+'</div>';
    return;
  }
  buildPinTabs();
  renderPinGroups();
