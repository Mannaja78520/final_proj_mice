...
// take effect. The RGB data pin is chosen from a SHORT LIST (config/rgb_pins
// .json -> RgbPins.h): FastLED fixes a pin at build time, so the firmware
// carries one instantiation per allowed pin and the page offers exactly those.
struct Pins {
    ...
    int rgbData;                     // WS2812 strip, one of RGB_PINS
...
public:
    ...
    String amp;                      // amp kind id from config/amps.json (e.g. "tpa3110", "max98357a")
    ...
    // Amplifier kind (stored in NVS namespace "hwpins" alongside pins)
    bool setAmp(const String& id);   // validate against AmpTable, store, return true if ok
    String getAmp() const { return amp; }
    void clearAmp();                 // revert to compile-time default
