...
#include <config.h>
#include "core/AmpTable.h"  // generated by gen_tables.py from config/amps.json
#if MICE_HAS_LIFT
#include "modules/lift/RgbPins.h"  // generated from config/rgb_pins.json
#endif
...
//
// Only the pins THIS build's module type actually has. A nong has no motor,
// so offering to wire one was an invitation to a mistake — and every one of
// those entries was also a PIN key, an NVS slot and a select box on the
// Setup page. The `Pins` struct keeps all its fields (a handful of ints, and
// the defaults stay valid); it is the *configurable* list that shrinks.
// The speaker (i2s_*) is wired on lift AND nong (A7-9).
struct Field {
    const char* key;
    const char* nvs;
    const char* label;
...
#if MICE_HAS_LIFT || MICE_HAS_NONG
#if MICE_HAS_LIFT
    {"rgb_data",  "rgbdat",  "RGB strip data", "rgb",  true},
#endif
...
#if MICE_HAS_LIFT
    if (key == "rgb_data")  return &pins.rgbData;
#endif
...
void HwConfig::begin() {
    ...
    // I2S pins: each module type defines its own NONG_/LIFT_I2S_* macros.
    // esp32_hardware.h includes both headers unconditionally, so bare I2S_BCLK_PIN
    // would collide (nong was silently overwriting lift's 27 with 2 since A7-9).
    // We pick the set that matches the built type.
#if MICE_HAS_LIFT
    pins.rgbData = RGB_PIN_DEFAULT;    // from config/rgb_pins.json
    pins.i2sBclk = I2S_BCLK_PIN;   // lift's own (27/14/2)
    pins.i2sLrc  = I2S_LRC_PIN;
    pins.i2sDout = I2S_DOUT_PIN;
#elif MICE_HAS_NONG
    pins.i2sBclk = NONG_I2S_BCLK_PIN;  // nong's (12/0/2)
    pins.i2sLrc  = NONG_I2S_LRC_PIN;
    pins.i2sDout = NONG_I2S_DOUT_PIN;
#else
    pins.i2sBclk = -1; pins.i2sLrc = -1; pins.i2sDout = -1;
#endif
#if !MICE_HAS_LIFT
    pins.rgbData = -1;                 // no strip on this type
#endif
    ...
    // Amplifier kind (stored under key "amp" in the same "hwpins" namespace)
    if (prefs_.isKey("amp")) {
        amp = prefs_.getString("amp");
    } else {
        // Compile-time default from the module's hardware header
#if MICE_HAS_LIFT
        amp = LIFT_AUDIO_AMP_DEFAULT;
#elif MICE_HAS_NONG
        amp = NONG_AUDIO_AMP_DEFAULT;
#else
        amp = "none";
#endif
    }
...
String HwConfig::validPinsJson() {
    ...
    for (int g = 0; g <= 39; g++) {
        ...
        if (g == hw.pins.rgbData) cls = "rgb";   // the chosen one, not a fixed 13
...
// Amplifier kind — validated against generated AmpTable.h
bool HwConfig::setAmp(const String& id) {
    if (!findAmp(id)) return false;      // unknown id
    prefs_.begin("hwpins", false);
    prefs_.putString("amp", id);
    amp = id;                            // struct updated now; audio re-init at reboot
    return true;
}

void HwConfig::clearAmp() {
    prefs_.begin("hwpins", false);
    prefs_.remove("amp");
    // revert to compile-time default
#if MICE_HAS_LIFT
    amp = LIFT_AUDIO_AMP_DEFAULT;
#elif MICE_HAS_NONG
    amp = NONG_AUDIO_AMP_DEFAULT;
