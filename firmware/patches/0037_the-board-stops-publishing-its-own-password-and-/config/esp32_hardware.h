...
//   config/esp32_hardware_cam_module.h    — ESP32-CAM sensor pins (fixed by the board)
    ...
    // LAST, and only on a camera build: it overrides the SD pins above,
    // because on that board they are the camera's own data lines.
