...
#modTabs{display:flex;gap:6px;margin:0 0 var(--sp-3)}
.mtab{flex:1;padding:9px 0;font-weight:600;background:color-mix(in srgb,var(--card) 60%,var(--bg));color:var(--mut);
  border:1px solid var(--line);border-radius:var(--r-sm);cursor:pointer;font-size:14px}
...
async function doLogin(){
  ...
  if(r.startsWith('OK') && !viaHub()){
    ...
    try{
      ...
      try{ mustChange = (await s.json()).mustChange === true; }catch(e){}
...
function doLogout(){
  ...
  if(!viaHub()) { try{ fetch('/api/logout',{method:'POST'}); }catch(e){} }
  auth=null;
