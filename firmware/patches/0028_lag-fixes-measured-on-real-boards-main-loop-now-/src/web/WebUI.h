...
// Live view: ask for the next frame as soon as the last one ARRIVES, never on
// a clock. A fixed 1 s timer was one frame a second on a board that delivers
// one every ~140 ms — six times slower than the hardware, for no reason. It
// also cannot overlap requests, so a slow link simply gets a slower view
// instead of a queue of stale frames.
function camLiveToggle(){
  if($('camLive').checked)camLoop();
}
async function camLoop(){
  if(camTimer)return;                 // already looping
  camTimer=1;
  try{
    while($('camLive').checked){
      const t0=performance.now();
      await camShot();
      // a small breath, so a fast board does not use the whole radio
      const spent=performance.now()-t0;
      if(spent<120)await new Promise(r=>setTimeout(r,120-spent));
    }
  }finally{ camTimer=null; }
}
