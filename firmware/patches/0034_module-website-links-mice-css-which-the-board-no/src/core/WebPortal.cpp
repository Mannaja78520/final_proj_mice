...
void WebPortal::begin(Identity* id, CommandRouter* router, SDStore* sd, RS485Bus* rs485) {
    ...
    // NO MODEM SLEEP.
    //
    // The ESP32 defaults to WIFI_PS_MIN_MODEM as a station: the radio dozes
    // between beacons and only wakes on the access point's DTIM, so a request
    // that arrives just after a beacon waits for the next one. Measured on the
    // bench before this line existed: a POSE over WiFi took a flat ~90 ms
    // median while the same command over USB took 25 ms — and dragging a
    // slider at 10 updates a second felt like exactly that.
    //
    // It costs power, which for a module bolted to a set and mains-powered is
...
void WebPortal::setupRoutes() {
...
#if MICE_HAS_CAM
    // ---- the live view: MANY frames down ONE connection ----------------
    //
    // A still costs ~25 ms to capture and ~120 ms to deliver, because this
    // server closes the connection after every response (measured: keep-alive
    // honoured 0 times out of 15). So a live view built from stills spends
    // five times longer opening sockets than taking pictures.
    //
    // An MJPEG stream pays that once. The response never ends: each part is a
    // boundary, a small header and one JPEG, produced a chunk at a time as the
    // TCP window allows.
    //
    // ONE viewer at a time. This board has four connections in total, and a
    ...
    server_.on("/api/cam.stream", HTTP_GET, [this](AsyncWebServerRequest* req) {
        ...
        // has produced nothing for three seconds is treated as gone.
        if (camStreaming_ && millis() - camStreamAt_ < 3000) {
            req->send(503, "text/plain",
                      "someone is already watching — one live view at a time");
            return;
        }
        camStreaming_ = true;
        camStreamAt_ = millis();
        // Per-response state: which frame we are sending and how far in. Freed
        // when the client goes away, which is the only way this ever ends.
        struct Stream {
            CamModule* cam = nullptr;
            camera_fb_t* fb = nullptr;
            String head;
            size_t sent = 0;      // bytes of the current part already produced
        };
        Stream* st = new Stream();
        st->cam = cam;
        AsyncWebServerResponse* res = req->beginChunkedResponse(
            "multipart/x-mixed-replace; boundary=mice",
            [this, st](uint8_t* buf, size_t maxLen, size_t) -> size_t {
                if (!st->fb) {                       // start the next part
                    st->fb = st->cam->take();
                    if (!st->fb) return RESPONSE_TRY_AGAIN;
                    st->head = "\r\n--mice\r\nContent-Type: image/jpeg\r\n"
                               "Content-Length: " + String((unsigned)st->fb->len) +
                               "\r\n\r\n";
                    st->sent = 0;
                }
                size_t hl = st->head.length();
                size_t total = hl + st->fb->len;
                size_t left = total - st->sent;
                size_t n = left < maxLen ? left : maxLen;
                if (!n) return RESPONSE_TRY_AGAIN;
                for (size_t i = 0; i < n; i++) {
                    size_t at = st->sent + i;
                    buf[i] = at < hl ? (uint8_t)st->head[at]
                                     : st->fb->buf[at - hl];
                }
                st->sent += n;
                camStreamAt_ = millis();   // this viewer is alive
                if (st->sent >= total) {             // part done: next frame
                    st->cam->give(st->fb);
                    st->fb = nullptr;
                }
                return n;
            });
        req->onDisconnect([this, st]() {
            if (st->fb) st->cam->give(st->fb);
            delete st;
            camStreaming_ = false;
        });
        req->send(res);
    ...
    server_.on("/api/ota", HTTP_POST,
        ...
        [this](AsyncWebServerRequest* req, String filename, size_t index,
            ...
            if (index == 0) {
                ...
                if (Update.isRunning()) {
                    Serial.println("[ota] an earlier upload never finished — dropping it");
                    Update.abort();
                }
                ...
                req->onDisconnect([]() {
                    if (Update.isRunning()) {
                        Serial.println("[ota] upload cut off — dropping it");
                        Update.abort();
                    }
                });
            }
