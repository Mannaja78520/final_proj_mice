...
public:
    ...
    // arg: "" | ON | OFF | AP | RETRY | SCAN. One line back, always.
...
private:
    ...
    // long enough for a module on our own AP, short enough that a stuck
    // peer cannot hold the servo loop for a noticeable moment
    static const uint16_t REACH_TIMEOUT_MS = 800;

    static WebPortal* self_;
    int scanState_ = 0;           // 0 idle, 1 a WIFI SCAN is running
