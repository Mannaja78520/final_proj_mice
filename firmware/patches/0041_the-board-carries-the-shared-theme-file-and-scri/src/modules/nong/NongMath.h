...
// M_PI is a POSIX extension: present on the ESP32 toolchain, absent from a
// standard-conforming host compiler. Define our own so this header builds
// anywhere, which is the whole point of it being separable.
static const float NONG_PI = 3.14159265358979323846f;
...
inline float ease(float t) {
    ...
    return 0.5f - 0.5f * cosf(NONG_PI * t);
}
