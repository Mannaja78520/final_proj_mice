...
void WebPortal::checkLink(int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!peers_.find(ssid, p)) continue;      // not a module we know of
        if (!bestRssi || rssi > bestRssi) { bestRssi = rssi; bestName = ssid; }
    }
...
void WebPortal::loop() {
    ...
    if (scanState_) {
        ...
        // scanning stopped the connect attempt; put it back the moment we can
        if (!scanState_ && wifiModeCfg_ == "on" && WiFi.status() != WL_CONNECTED) {
            WiFi.begin(staSsid_.c_str(), staPass_.length() ? staPass_.c_str() : nullptr);
        }
    }
    ...
    if (wifiModeCfg_ == "on" && serverStarted_ && !scanState_ &&
        ...
        if (onRelay_ || rssi == 0 || rssi < wifilink::WEAK_RSSI) {
            WiFi.scanDelete();
            WiFi.disconnect(false, false);   // a connect in flight blocks a scan
            if (WiFi.scanNetworks(true) != WIFI_SCAN_FAILED) scanState_ = 2;
        }
    }
