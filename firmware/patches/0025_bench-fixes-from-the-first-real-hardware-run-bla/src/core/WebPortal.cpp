...
void WebPortal::setupRoutes() {
    ...
    server_.on("/api/cam.jpg", HTTP_GET, [this](AsyncWebServerRequest* req) {
        ...
        // The driver's own buffer goes out as-is; nothing is copied (see
        // CamModule::take). It is handed back only once the send is finished,
        // or the driver would refill it under the response.
        AsyncWebServerResponse* res =
            req->beginResponse_P(200, "image/jpeg", fb->buf, fb->len);
        res->addHeader("Cache-Control", "no-store");
        req->onDisconnect([cam, fb]() { cam->give(fb); });
        req->send(res);
    });
#endif
