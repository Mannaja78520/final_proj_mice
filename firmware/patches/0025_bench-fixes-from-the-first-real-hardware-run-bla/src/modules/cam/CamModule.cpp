...
// Take a frame, and give it back. NOTHING copies it.
//
// The first version copied every frame into a malloc'd buffer so the async web
// server could send it after the capture returned. On the bench that turned out
// to be the one thing this board cannot do: on this ESP32-CAM a plain
// malloc(16 KB) PANICS inside the allocator, before the camera is touched at
// all — psramFound() says yes, but the PSRAM heap reports 0 bytes free, so the
// allocator walks a heap region that was never registered.
//
// The camera driver's own buffers are fine (it allocates them at init, through
// its own path), and a captured frame is perfectly good. So the module never
// allocates: it hands out the driver's buffer and takes it back when the send
// is done. That is fewer copies, less RAM, and it works on a board whose heap
// cannot be trusted with a 16 KB request.
