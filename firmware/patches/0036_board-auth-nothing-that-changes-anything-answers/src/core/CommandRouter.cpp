...
String CommandRouter::handle(const String& line) {
    ...
    //
    // AFTER the command, not before, and only if the module actually took it.
    // A binary can carry a type the board is not currently set to (flash the
    // nong firmware, leave the board on `blank`), and then POSE is a motion
    // command that nothing can execute — stopping a running show for it would
    // end the show and put nothing in its place. Found on a real board.
    // Nothing can interleave: the sequence player steps from router.loop(),
    // under this same recursive lock.
    String reply = handleLocked(line);
    bool stopped = reply.startsWith("ERR unknown cmd") ? false
                                                       : preemptSequence(line);
