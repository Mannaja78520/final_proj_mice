...
function camShot(){
  const img=$('camImg');
  img.style.display='';
  img.onerror=()=>{$('camStat').textContent='no picture — is the ribbon cable seated?';};
  img.onload=()=>{$('camStat').textContent='picture taken '+new Date().toLocaleTimeString();};
  img.src='/api/cam.jpg?'+Date.now();
}
function camLiveToggle(){
  if(camTimer){clearInterval(camTimer);camTimer=null;}
  if($('camLive').checked){camShot();camTimer=setInterval(camShot,1000);}
}
