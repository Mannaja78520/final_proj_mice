...
// The sensor clock.
//
// 20 MHz is the figure every ESP32-CAM example uses, and on this board it
// produced single-pixel coloured lines scattered through every frame — whole
// scanlines arriving corrupted, which looks like sensor noise and is not. The
// camera's parallel bus is being clocked faster than this board's wiring can
// carry reliably.
//
// 10 MHz halves the frame rate ceiling and removes them. Frame rate was never
// the constraint here: the live view is limited by WiFi, not by the sensor.
#define CAM_XCLK_HZ 10000000

// The shared core's SD defaults (5, 18, 19, 23) are THIS board's camera data
// lines. There is no SPI card on an ESP32-CAM — its socket is on the SDMMC bus
// (2, 4, 12-15), which the camera also needs — so the SPI card is disabled
// here rather than left pointing at the camera. Overriding the shared defaults
// is the whole reason this file is included after them.
#undef SD_CS_PIN
#undef SD_SCK_PIN
#undef SD_MISO_PIN
#undef SD_MOSI_PIN
