...
public:
    ...
    // The same, for steps coming FROM the sequence player.
    //
    // THERE IS ONLY EVER ONE THING DRIVING THE SERVOS. A board can play a
    // sequence on its own clock (that is what lets a show survive the browser
    // being closed) while an app sends poses down the cable at the same time —
    // and then both drive the same joints and the moves overlap: the arm
    // fights itself, and stopping the app does not stop the board.
    //
    // So a MOTION command from outside stops the running sequence first (see
    // handle()). The player's own steps must not do that to themselves, which
    // is the whole difference between these two entry points.
    String handleFromSequence(const String& line);
