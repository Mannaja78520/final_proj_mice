...
String CommandRouter::handleLocked(const String& line) {
    ...
    if (cmd == "SET") {
        ...
        if (what == "WIFI") {
            ...
            String ssid, pass;
            String rest = Util::joinFrom(argv, argc, 2);
            if (rest.startsWith("\"")) {
                int end = rest.indexOf('"', 1);
                if (end < 0) return "ERR unclosed quote — SET WIFI \"my ssid\" [pass]";
                ssid = rest.substring(1, end);
                pass = rest.substring(end + 1);
                pass.trim();
            } else {
                ssid = argv[2];
                pass = argc >= 4 ? Util::joinFrom(argv, argc, 3) : "";
            }
            if (!ssid.length()) return "ERR need an ssid";
