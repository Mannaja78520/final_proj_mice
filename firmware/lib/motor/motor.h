...
class Controller
{
    public:
        ...
        void brake()
        {
            ...
            {
                case Drive2pin:
                    // full-scale duty on both inputs = short brake; HIGH (1) out of
                    // 2^pwm_bits levels is ~0% duty and would coast instead
                    analogWrite(motor_pinA_, brakemotor_ ? ((1 << pwm_bits_) - 1) : 0);
                    analogWrite(motor_pinB_, brakemotor_ ? ((1 << pwm_bits_) - 1) : 0);
                    break;
