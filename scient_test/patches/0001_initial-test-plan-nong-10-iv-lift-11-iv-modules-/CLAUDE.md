...
- `tools/bench.py` is **stdlib-only for WiFi mode**; `pyserial` may be required
  for USB/RS485 mode but must fail with a clear message when missing. Keep it
  runnable with a bare `python tools/bench.py`.
