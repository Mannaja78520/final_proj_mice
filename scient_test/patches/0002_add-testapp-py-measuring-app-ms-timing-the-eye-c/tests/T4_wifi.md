...
> **Every DV here is a millisecond or a percentage — the app measures all of
> them.** `python ../tools/bench.py wifi --host <ip> --cmd PING -n 50` (or
> `testapp.py latency --transport wifi …`) gives latency and loss for WIFI-01,
> 02, 04, 05, 09. What you do by hand is move the module, move the walls,
> switch the AP on and off, and read `wifi.rssi` from
> `testapp.py info --transport wifi --host <ip>`. The one thing that still
> needs a stopwatch is the slow stuff — AP fallback (~15 s), late join (~60 s),
> reconnect — and those are seconds, so a stopwatch is fine there.
