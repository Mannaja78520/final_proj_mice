...
> **The millisecond rows are measured by the app, never by eye.** NONG-03, 04
> and 05 are all sub-second: run them with
> `python ../tools/testapp.py nong-sweep --transport usb --port COM5 --iv speed|T|maxdps --values …`
> which reports commanded `T`, reported `T`, **executed ms** and the 20 ms
> update tick, and writes the rows for you. What stays yours to measure is
> everything that holds still: **the joint angle with a gauge** (NONG-01, 02,
> 06, 07, 09, 10), current and rail voltage (NONG-08), and a 60 fps video for
> the two-board sync in NONG-11b. See [README.md](README.md) § "You cannot see
> milliseconds".
