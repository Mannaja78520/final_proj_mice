...
function poseChanged(throttled) {
  ...
  if ($("liveChk").checked) sendPoseLive();
}
...
function renderTimeline() {
  ...
  keys.forEach((k, i) => {
    ...
    tIn.onchange = () => {
      ...
      delete k.dps;             // a typed time wins over a speed override
      if (want < kmin)
...
function playKeys() {
  ...
  keys.forEach((k, i) => {
    ...
    let t = k.t;
    if (prev && gap) {
      // This move is NOT the one its stored time was written for — it now
      // starts from an earlier pose, so it covers more ground. Re-time it at
      // the SPEED that applies (this keyframe's own °/s, else the sequence's),
      // never merely at the servos' physical floor: floor is the fastest the
      // arm can possibly move, so using it made a suspended keyframe turn the
      // replacement move into a full-speed lunge.
      // max() with the stored time so suspending can only ever slow a move
      // down, never speed one up that was deliberately made slow.
      t = Math.max(k.t, autoTime(prev.pose, k.pose, k.dps || speedDps()));
    }
  ...
  try {
    ...
    for (let i = 0; i < NJ; i++) {
      ...
      if (!(same("min", i, mn) && same("max", i, mx)))
        todo.push(`LIMIT ${i + 1} ${mn} ${mx}`);
      if (!(same("gear_pinion", i, RIG.gearPinion[i]) && same("gear_gear", i, RIG.gearGear[i])))
        todo.push(`GEAR ${i + 1} ${RIG.gearPinion[i]} ${RIG.gearGear[i]}`);
      if (!(same("pulse_min", i, pmin) && same("pulse_max", i, pmax) && same("max_dps", i, dps)))
        todo.push(`PULSE ${i + 1} ${pmin} ${pmax} ${dps}`);
      if (!same("servo_range", i, rng)) todo.push(`RANGE ${i + 1} ${rng}`);
      if (!same("frame_hz", i, hz)) todo.push(`RATE ${i + 1} ${hz}`);
    }
    if (!todo.length) {
    ...
    for (let k = 0; k < todo.length; k++) {
      $("limStat").textContent = `sending ${k + 1}/${todo.length}: ${todo[k]}`;
      await rawCmd(todo[k]);
    }
    $("limStat").textContent =
      `sent ${todo.length} change${todo.length > 1 ? "s" : ""} ` +
      `(limits, gear, servo pulse, travel, frame rate)` +
    ...
    try {
      for (;;) {
        ...
        if (liveQueue.length) {
          ...
          c = "POSE " + livePending.map(fmtA).join(" ");
  ...
  if (playing) {
    ...
    if (liveLinked()) {
      ...
      if (seg !== -1 && seg !== lastLiveSeg) {
        ...
        const t = K[seg].t - rem <= 50 ? K[seg].t : rem;
        liveSend("POSE " + K[seg].pose.map(fmtA).join(" ") + " T " + t);
      }
    }
  }
  lastFrame = now;
