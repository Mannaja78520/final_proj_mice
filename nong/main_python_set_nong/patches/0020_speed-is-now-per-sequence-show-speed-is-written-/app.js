...
function playKeys() {
  ...
  keys.forEach((k, i) => {
    ...
    out.push({
      pose: k.pose, hold: k.hold || 0, src: i,
      t: prev ? Math.max(k.t, minTime(prev.pose, k.pose)) : k.t,
    });
  });
  return out;
}
...
function buildYaml() {
  ...
  lines.push(`  - speed: ${Math.round(speedDps())}`);
...
function parseSeqYaml(text) {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if ((m = line.match(/^name:\s*(.+)$/))) out.name = m[1].trim();
      ...
      out.speed = +m[1];        // this sequence's own show speed — not "skipped"
