...
function keysSignature() {
  ...
  for (let i = 0; i < keys.length; i++) {
    const k = keys[i];
    s += ";" + (k.off ? 1 : 0) + "," + k.t + "," + (k.hold || 0) + "," +
         (k.dps || 0) + "," + k.pose;
  }
  return s;
}
function bumpKeys() { _pkCache = null; }   // kept for explicit invalidation
function playKeys() {
...
document.addEventListener("visibilitychange", function () {
  if (document.hidden) handOffToRobot();
  else handedOff = false;                    // back on the page: allow it again
});
...
if (window.ResizeObserver) {
  new ResizeObserver(function () { resize(); }).observe(viewport);
}
