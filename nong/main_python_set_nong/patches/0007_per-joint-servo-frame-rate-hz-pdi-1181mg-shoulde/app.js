...
const DEFAULT_RIG = {
  ...
  // SERVO FRAME RATE per joint (Hz): 50 for normal hobby servos, 330 for the
  // PDI-1181MG digital shoulders. A wrong rate can make a digital servo cut
  // torque ("disable itself"). Pushed to the robot with the RATE command.
  frameHz: [330, 330, 50, 50, 330, 330, 50, 50, 50, 50],
...
const SERVO_TYPES = {
  ...
  pdi1181mg:  { label: "PDI-1181MG", min: 500, max: 2500, dps: 375, range: 270, hz: 330 },
...
function runServoSelfTest() {
  ...
  // FRAME RATE: the PDI-1181MG preset is 330 Hz, MG90S 50 Hz; a preset must
  // carry the rate and only touch its group.
  RIG.frameHz = new Array(NJ).fill(50);
  applyServoPreset("pdi1181mg|sh");
  if (!shIdx.every(i => RIG.frameHz[i] === 330))
    fails.push("pdi1181mg preset did not set 330 Hz on the shoulders");
  if (!elIdx.every(i => RIG.frameHz[i] === 50))
  ...
  document.title = fails.length
    ...
    : `SERVO PASS slowJointStretched=${fastElbow}->${slowElbow}ms otherMoveUnchanged=${shAfter}ms presets=perGroup travel=ok frameRate=pdi330/mg50`;
