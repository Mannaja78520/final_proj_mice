...
const JOINT_LABELS = ["L shoulder pitch", "L shoulder roll", "L elbow pitch", "L elbow roll",
                      ...
                      "Waist (turn L/R)", "Shrug (shoulders up/down)"];
...
const DEFAULT_RIG = {
  ...
  dims: { shoulderX: 105, shoulderY: 95,
          upperLenL: 115, upperLenR: 115, foreLenL: 105, foreLenR: 105,
          torsoW: 170, torsoH: 240, torsoD: 90 },
  ...
  // jointR = radius of that joint's ball, barR = radius (thickness) of the
...
function mergeRig(saved) {
  ...
  ["min", "max", "axis", "invert", "neutral",
   "gearPinion", "gearGear", "pulseMin", "pulseMax", "servoMaxDps", "servoRange"].forEach(fixLen);
...
function buildArm(side) {           // side: +1 = robot left (+X), -1 = right
  ...
  anchor.position.set(side * d.shoulderX, d.shoulderY, 0);
  shoulderMount.add(anchor);        // SHRUG lifts the mount; WAIST yaws the body
...
function buildRobot() {
  ...
  // the shrug lifts only the shoulder mount. Both are visual here — the real
  // joint value is what gets sent to the robot.
  bodyGroup = new THREE.Group(); robot.add(bodyGroup);
  shoulderMount = new THREE.Group(); bodyGroup.add(shoulderMount);
...
function applyPose() {
  ...
  for (let i = 0; i < ARMJ; i++) {
    ...
    if (!g) continue;
    const ax = RIG.axis[i];
    const v = (pose[i] - RIG.zero[i]) * jdir(i);
    const t = RIG.tilt[i] || [0, 0, 0];
    _eTilt.set(THREE.MathUtils.degToRad(t[0]), THREE.MathUtils.degToRad(t[1]),
               THREE.MathUtils.degToRad(t[2]), "XYZ");
  ...
  if (bodyGroup) {
    const wy = (pose[8] - RIG.zero[8]) * jdir(8);
    bodyGroup.rotation.set(0, THREE.MathUtils.degToRad(wy), 0);
  }
  if (shoulderMount) {
    const sh = (pose[9] - RIG.zero[9]) * jdir(9);
    // exaggerate x3 so the small (~6°) shrug is visible in the model
    shoulderMount.rotation.set(THREE.MathUtils.degToRad(sh * 3), 0, 0);
  }
}
...
function renderRigUI() {
  ...
  svh.textContent = "servo & gear per joint (pinion:gear, pulse µs, max °/s, travel°):";
  jb.appendChild(svh);
  ...
  JOINT_LABELS.forEach((label, i) => {
    const row = document.createElement("div"); row.className = "rigjrow";
    row.style.gridTemplateColumns = "78px 42px 42px 54px 54px 48px 48px";
    const name = document.createElement("span");
    name.className = "jname"; name.textContent = label;
  ...
  Object.entries(DIM_LABELS).forEach(([k, label]) => {
    ...
    const name = document.createElement("span"); name.className = "jname"; name.textContent = label;
    const inp = document.createElement("input");
    inp.type = "number"; inp.min = 5; inp.max = 1000; inp.value = RIG.dims[k];
    inp.onchange = () => { RIG.dims[k] = Math.max(5, +inp.value || RIG.dims[k]); rigChanged(); };
...
const SERVO_TYPES = {
  mg90s:      { label: "MG90S",      min: 500, max: 2400, dps: 400, range: 180 },
  pdi1181mg:  { label: "PDI-1181MG", min: 500, max: 2500, dps: 375, range: 270 },
  tiankong35: { label: "TianKongRC 35kg", min: 500, max: 2500, dps: 250, range: 270 },
  generic180: { label: "generic 180", min: 500, max: 2500, dps: 300, range: 180 },
  generic270: { label: "generic 270", min: 500, max: 2500, dps: 300, range: 270 },
};
...
async function pushLimits() {
  ...
  try {
    ...
    $("limStat").textContent = "sent per-joint limits, gear, servo pulse and travel to the robot (saved on its SD card) ✓";
...
//   usb  — Web Serial, 115200 baud. With an empty "bus id" the cable goes
//          straight into the nong module's USB. With a bus id set, every
//          command is framed "#<id> ..." and replies "@<id> ..." — that is
//          the RS485 wire protocol, so the same mode covers BOTH a
//          USB-RS485 dongle on the bus AND reaching the nong through any
//          other module's USB port (modules bridge '#' lines onto the bus).
...
function connModeChanged() {
  ...
  $("robotIp").style.display = usb ? "none" : "";
  $("busId").style.display = usb ? "" : "none";
  $("connBtn").textContent = usb && !serialPort ? "Connect USB" : "Connect";
  if (usb && !("serial" in navigator))
    $("robotStat").textContent = "this browser has no Web Serial — use Chrome/Edge, or WiFi mode";
}
  ...
  try {
    ...
    for (;;) {
      ...
      while ((i = buf.indexOf("\n")) >= 0) {
        ...
        buf = buf.slice(i + 1);
        if (!line || line.startsWith("[")) continue; // skip boot/debug log lines
...
function haveUsb() { return !!(serialPort && serialPort.writable); }
...
async function rawCmd(c) { // reply text or throws
  if (haveUsb()) return serialCmd(c);
  if (haveWifi()) return httpCmd(c);
  throw new Error("connect first — 🔍 Find modules (WiFi) or Connect USB");
}
...
async function getStatus() { // full status JSON on whichever link is up
  if (haveUsb()) {
    try { return JSON.parse(await serialCmd("INFO")); }
    catch (e) { return JSON.parse(await serialCmd("INFO")); } // boot noise: retry once
  }
  if (haveWifi())
...
function linkBadge() { // shown in robotStat so you see every open channel
  const parts = [];
  if (haveUsb()) parts.push("USB" + (busId() ? "→RS485 #" + busId() : "") + " ✓");
...
async function connectRobot() {
  try {
    if (transport() === "usb" && !haveUsb()) await serialConnect();
    ...
    refreshSd();
  } catch (e) { $("robotStat").textContent = "cannot connect: " + (e.message || e); }
}
...
async function sdUploadSerial(fname, text) {
  ...
  let r = await serialCmd("FBEGIN " + fname);
...
async function sdDownloadSerial(fname) {
  const parts = [];
  for (let off = 0; ; ) {
    const r = await serialCmd(`FREAD ${fname} ${off} 120`);
    if (r === "EOF") break;
...
async function sdDelete(fname) {
  ...
  const r = await serialCmd("FDEL " + fname);
  if (!r.startsWith("OK")) throw new Error(r);
}
...
async function refreshSd() {
  ...
  try {
    ...
    if (haveWifi()) {
      ...
      files = JSON.parse(await serialCmd("FILES /moves"));
...
function tick(now) {
  ...
  if (playing) {
    ...
    if ($("liveChk").checked) {
      ...
      if (seg !== -1 && seg !== lastLiveSeg && robotIp()) {
        lastLiveSeg = seg;
        robotCmd("POSE " + keys[seg].pose.map(fmtA).join(" ") + " T " + keys[seg].t);
      }
    }
  }
  lastFrame = now;
...
function runServoSelfTest() {
  ...
  RIG.gearPinion = save.gp; RIG.gearGear = save.gg;
  rigChanged(); renderRigUI();
  document.title = fails.length
    ? "SERVO FAIL " + fails.join(" | ")
    : `SERVO PASS slowJointStretched=${fastElbow}->${slowElbow}ms otherMoveUnchanged=${shAfter}ms presets=perGroup travel180flagged=true travel270ok=true`;
...
function runIkSelfTest() {
  ...
  const msg = `IKTEST ${ok ? "PASS" : "FAIL"} reached ${pass}/${poses.length} worst=${worst.toFixed(1)}mm errs=[${errs}] shoulderMoves=${shOK} elbowMoves=${elOK}`;
  document.title = msg;
