...
function poseChanged(throttled) {
  ...
  if ($("liveChk").checked) {
    // 70 ms ~= 14 poses/s. The old 180 ms was sized for a ~250 ms round trip;
    // a command now costs ~30 ms over the hub, so the link keeps up easily and
    // dragging a slider reads as live instead of stepping.
    if (!throttled || now - liveTimer > 70) { liveTimer = now; sendPose(true); }
  }
}
...
function segmentAt(ms) {
  ...
  for (let i = 1; i < keys.length; i++) {
    if (ms < t + keys[i].t) return i;          // moving into keyframe i
